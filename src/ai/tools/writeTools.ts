...
export const prepareTransferTool: ToolDefinition = {
	...
	execute: async (params, context): Promise<ToolResult> => {
		...
		console.log(
			`[prepare_transfer] On-chain isWrapped: ${ownerInfo.isWrapped}`,
		);
		...
		try {
			...
			const { getTransferService } = await import(
				"../../services/ens/transfer/transfer"
			);
...
Call this AFTER step 1 transaction is signed and confirmed.`,
	...
	execute: async (params, context): Promise<ToolResult> => {
		...
		try {
			...
			const { getSession, setSessionPendingAction } = await import(
				"../sessions"
			);
			...
			const _isWrapped = subData.isWrapped as boolean;
...
export const prepareBridgeTool: ToolDefinition = {
	...
	execute: async (params, context): Promise<ToolResult> => {
		...
		try {
			...
			// Import utilities
			const { getBridgeQuoteAndTx } = await import(
				"../../services/bridge/bridge"
			);
			const { CHAIN_IDS } = await import(
			...
			const mainnetBalanceCheck = await checkBalance(
				walletAddress,
				CHAIN_IDS.MAINNET,
			);
...
Call verify_ownership first to confirm the user owns the name.`,
	...
	execute: async (params, context): Promise<ToolResult> => {
		...
		try {
			...
			const { encodeFunctionData } = await import("viem");
			const { ENS_CONTRACTS, REVERSE_REGISTRAR_ABI } = await import(
				"../../services/ens/constants"
			);
